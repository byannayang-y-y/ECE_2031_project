...
architecture a of SCOMP is
	type state_type is (
		...
		ex_loadi, ex_add, ex_addi, ex_sub, --add a sub state here
		ex_and, ex_or, ex_xor, ex_shift,
		ex_jump, ex_jneg, ex_jzero, ex_jnz, ex_jpos, --add jnz and jpos states here
		ex_return, ex_call,
...
begin
	...
	GENERIC MAP (
		...
		widthad_a => 11,
		width_a => 16,
		init_file => "MaskedReactionGame.mif",
			...
			case state is
				...
				when decode =>
					...
					case mem_data(15 downto 11) is -- opcode is top 5 bits of instruction
						...
						-- add jnz					-- jump not zero
						when "01111"  =>
							state <= ex_jnz;

						-- add jpos     			-- jump if positive
						when "01101"  =>
							state <= ex_jpos;
				...
				-- add jpos execution        -- jump if positive
				when ex_jpos =>
				...
				-- add jnz execution         -- jump not zero
				when ex_jnz =>
					if (AC /= x"0000") then
						PC    <= operand;
					end if;
